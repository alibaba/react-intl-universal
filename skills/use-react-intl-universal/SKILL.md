...
When the user asks to start a UI inspection or provides a page URL for localization QA, open [UI Inspection Mode](references/ui-inspection-mode.md) and follow its state-machine workflow instead of the daily development workflow. Before capturing finding evidence or populating the report, also open [UI Inspection Evidence](references/ui-inspection-evidence.md); it is the single contract for capture-candidate review, screenshot admission, annotations, before/after binding, and preserving the copied report wireframe. Populate the copied wireframe from `report.json` with `scripts/render-ui-inspection-report.mjs`; do not keep a second hand-authored report data model in HTML.
For broad, full-navigation, release-quality, or high-assurance inspections, use the optional independent screenshot review described in UI Inspection Mode when subagents are available, then triage reviewer observations through the same finding-admission gates.
...
- `references/ui-inspection-evidence.md`: open before accepting screenshots or editing `report.json`/`report.html`; it defines evidence admission and the wireframe-preservation contract.
...
- `references/ui-inspection-report-wireframe.html`: structural HTML reference for the human-readable `report.html` generated by UI Inspection Mode.
- `scripts/render-ui-inspection-report.mjs`: deterministically populates the wireframe from admitted `report.json` records while preserving template-owned styling and interactions.
